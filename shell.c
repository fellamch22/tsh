...
/*Fonction pour: rm ,rm -r et rmdir
 * trouveBis(int fd, char *filename)
 *delete_fichier(int fd, char *filename)
 *delete_repertoire(int fd, char *repname)
 */


static off_t trouveBis(int fd, char *filename){
  int filesize = 0;
  struct posix_header p;
  lseek(fd, 0, SEEK_SET);
...
static void delete_fichier(int fd, char *filename){
  int filesize = 0;
  off_t position , pos_decalage;
  off_t diff;
  struct stat s ;
  struct posix_header p;
  position = trouveBis(fd, filename);
  if(position == -1){
...
static void delete_repertoire(int fd, char *repname){
